...
import DAO.MedicosConsultasDAO;
 ...
 * @author yugurimaru
 */
public class Consultas_procurar_medico extends javax.swing.JFrame {

    ArrayList<MedicoConsultas> MedicoConsultas = new ArrayList();
    ArrayList<Medico> medico = new ArrayList();
    
    /**
     * Creates new form Consultas_procurar_data
     */
    public Consultas_procurar_medico() {
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        Tabela.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID Consulta", "Nome Médico", "Nome Paciente", "Data Consulta", "Hora Consulta", "Especialidade", "Sexo", "Data Nascimento", "Telefone"
            }
        ) {
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        for (int i = 0; i < medico.size(); i++) {
            CB_medico.addItem(medico.get(i).getNome_med());
        }
    ...
    private void Atualizar_tabela() {
        ...
        Tabela.removeAll();
        String id_medico, id_paciente, id_consulta, data_string, hora_string, compareceu, nome, sexo, data_nascimento, telefone, especialidade;
        ...
        MedicosConsultasDAO MCDAO;
        try {
            MCDAO = new MedicosConsultasDAO();
            
            //Seleciona o medico da ComboBox
            Medico medico_selecionado = medico.get(CB_medico.getSelectedIndex());
            int codigo_medico = medico_selecionado.getID_Medico();
            
            MedicoConsultas = MCDAO.listar(codigo_medico);
        } catch (NumberFormatException e) {
            System.out.println("Erro de conexão ao banco.");
        }

        for (int i = 0; i < MedicoConsultas.size(); i++) {
            
            int id_medico_procurar = MedicoConsultas.get(i).getId_medico();
            MedicoDAO MDAO = new MedicoDAO();
            Medico medico = MDAO.Procurar(id_medico_procurar);
            id_medico = medico.getNome_med();
            
            int id_paciente_procurar = MedicoConsultas.get(i).getId_paciente();
            PacienteDAO PDAO = new PacienteDAO();
            Paciente paciente = PDAO.procurar(id_paciente_procurar);
            id_paciente = paciente.getNome_pac();
            
            id_consulta = Integer.toString(MedicoConsultas.get(i).getID_consultas());

            data_string = dtOutput.format(MedicoConsultas.get(i).getDate());

            SimpleDateFormat format = new SimpleDateFormat("HH:mm:ss");
            Time hora;

            try {
                hora = MedicoConsultas.get(i).getHora();
            ...
            nome = MedicoConsultas.get(i).getNome_med();
            especialidade = MedicoConsultas.get(i).getEspecialidade();
            sexo = MedicoConsultas.get(i).getSexo() + " ";
            data_nascimento = dtOutput.format(MedicoConsultas.get(i).getData_nasc());
            telefone = MedicoConsultas.get(i).getFone();
            

            table.addRow(new String[]{id_consulta, id_medico, id_paciente, data_string, hora_string, especialidade, sexo, data_nascimento, telefone, });
        }
    }
